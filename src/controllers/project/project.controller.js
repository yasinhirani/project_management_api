...
import { AllocatedHoursDetail } from "../../models/allocatedHoursDetails/allocatedHoursDetails.model.js";
...
const deleteProject = asyncHandler(async (req, res, next) => {
  ...
  if (project.projectDocuments.length > 0) {
    for (let i = 0; i < project.projectDocuments.length; i++) {
      await deleteFiles(
        project.projectDocuments[i].public_id,
        cloudinaryFolderPath.PROJECT
      );
    }
  }
 ...
 */
// const assignResources = asyncHandler(async (req, res, next) => {
...
const assignResources = asyncHandler(async (req, res, next) => {
  ...
  if (resources.length > 0) {
    ...
    updatedResources = resources.map((resource, index) => {
      return {
        employeeDetail: resource._id,
